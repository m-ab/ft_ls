...
static void			file_l(t_info *f) // remplacer les putchar(' ') par une fonction qui forme les colonnes "parfaites"
{
	...
	ft_putchar(' ');
	S_ISLNK(f->file_stat.st_mode) ?
	ft_putstr(f->dir_data->d_name) : ft_putendl(f->dir_data->d_name);//file name
}
...
void				display_files_l(t_env *env, t_info *begin, int i)
{
	t_info	*tmp;
	t_info	*tmp2;
	...
	{
		if (env->l && S_ISLNK(tmp->file_stat.st_mode))
		{
			file_l(tmp);
			ft_putstr(" -> ");
	...
	{
		if (tmp2->down)
			display_files_l(env, tmp2->down, 0);
		tmp2 = tmp2->next;
	}
}
